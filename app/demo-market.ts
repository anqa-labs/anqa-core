...
async function main() {
  ...
  await step("market + book", () => exists(market), () =>
    p.methods
      .initializeMarket(MARKET_ID, new BN(TICK), new BN(1), 8, DEC, 0, 0, { pyth: {} }, {
...
/*
 * KNOWN GAP (2026-07-31): markets provisioned by *this* script settle fills
 * with `LockActive`, while markets provisioned by `app/per-e2e.ts` settle
 * fine — same program, same keeper, same accounts, and the lit path fails
 * identically on a bad market, so it is not the dark code.
 *
 * Ruled out by inspection on a failing market: clock drift (converged, 17
 * slots), market mode (Live), group locks (bankruptcy/stress/loss-stale all
 * clear), account health (valid certs, matching epochs, no stale or b-stale
 * flags), asset lifecycle (Active) and both side modes (Normal), and the
 * keeper itself (fails with it stopped too).
 *
 * Until the difference is found, use `ANQA_DEMO_MARKET=<id> npx ts-node
 * --transpile-only app/per-e2e.ts` to provision the venue — it is the proven
 * path and now persists its mint for the terminal's faucet — then
 * `app/demo-maker.ts` for depth.
 */
