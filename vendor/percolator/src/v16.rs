...
// ANQA MODIFICATION (2026-07-30): 16 -> 4.
//
// This constant sizes `PortfolioAccountV16Account.legs` and, via
// PORTFOLIO_SOURCE_DOMAIN_CAP (= 2N), its `source_domains`. At 16 the account
// is 9,291 bytes, of which ~75% is capacity for assets Anqa does not trade.
//
// Every trader's portfolio is delegated into an ephemeral rollup, and commit
// cost scales with account size — so the unused capacity is a tax on every
// commit, per trader, forever. Anqa trades three assets (BTC, ETH, SOL); 4
// leaves one spare slot.
//
// Upstream anticipates this: the compile-time size assertion below and
// V16_LAYOUT_DISCRIMINATOR exist to make the change deliberate rather than
// accidental. Both are updated to match.
...
// Bumped from 17 alongside the portfolio resize above — the layout changed,
// so accounts written under the old shape must not deserialize under the new.
pub const V16_LAYOUT_DISCRIMINATOR: u16 = 1017;
...
#[cfg(not(kani))]
const _: () = assert!(core::mem::size_of::<PortfolioAccountV16Account>() == 2859);
